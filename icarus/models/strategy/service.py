...
class Coordinated(Strategy):
    ...
    def find_topmost_feasible_node(self, receiver, flow_id, path, time, service, deadline, rtt_delay):
        ...
        for n in reversed(path[1:-1]):
            ...
            if deadline - time - rtt_to_cs < serviceTime:
                continue
            aTask = Task(time, Task.TASK_TYPE_SERVICE, deadline, rtt_to_cs, n, service['content'], service['labels'],
                         serviceTime, flow_id, receiver, time + delay)
    ...
    def replace_services(self, time):
            ...
            for node in nodes:
                ...
                for service, util in service_utils_sorted:
                    ...
                    if self.debug and self.numVMsPerService[cs.node][service] > 0:
                        ...
                        print((str(self.numVMsPerService[cs.node][service]) + " vms are instantiated at node: " + str(
                            cs.node) + " for service: " + str(service)))
                ...
                while remaining_vms > 0:
                    ...
                    for service, util in service_utils_sorted:
                        ...
                        if (self.numVMsPerService[cs.node][service] > 0):
                            ...
                            remaining_vms -= 1
                            if self.debug:
                                print(("One additional vm is instantiated at node: " + str(
                                    node) + " for service: " + str(service)))
                        else:
                                ...
                                for recv in self.receivers:
                                    ...
                                    if self.serviceNodeUtil[ap][node][service] == 0:
                                        ...
                                        for n in path[1:]:
                                            self.serviceNodeUtil[ap][n][service] = 0.0
                                if self.debug:
                                    print((str(num_vms) + " additional vm is instantiated at node: " + str(
                                        node) + " for service: " + str(service)))
                        if remaining_vms == 0:
                            break
        ...
        for node in list(self.compSpots.keys()):
            ...
            if len(servicesToReplace) != len(servicesToAdd):
                print(("Error: Services to replace " + str(servicesToReplace) + " is different in size from: " + str(
                    servicesToAdd)))
    ...
    # Coordinated            
    @inheritdoc(Strategy)
    def process_event(self, time, receiver, content, labels, log, node, flow_id, deadline, rtt_delay, status,
                      task=None):
        """
        ...
        service = content
        if self.debug:
            print(("\nEvent\n time: " + repr(time) + " receiver  " + repr(receiver) + " service " + repr(
        ...
        if receiver == node and status == REQUEST:
            ...
            for thisTask in compSpot.scheduler.upcomingTaskQueue:
                ...
                for thisTask in compSpot.scheduler._taskQueue:
                    ...
                    raise ValueError("No task with the given flow id: " + str(
                        flow_id) + " in the upcomingTaskQueue or TaskQueue of node: " + str(node))
            newTask = compSpot.scheduler.schedule(time)
            if newTask is not None:
                self.controller.add_event(newTask.completionTime, newTask.receiver, newTask.service, newTask.labels,
                                          compSpot.node, newTask.flow_id, newTask.expiry, newTask.rtt_delay,
            ...
            self.controller.add_event(time + delay, receiver, service, labels, receiver, flow_id, deadline, rtt_delay,
                                      RESPONSE)
            if self.debug:
                print(("Flow id: " + str(flow_id) + " is scheduled to arrive at the receiver at time: " + str(
                    time + delay)))
            if (time + delay > deadline):
                # if node == 5:
                print((
                                  "Error in COORDINATED strategy: Request missed its deadline\nResponse at receiver at time: " + str(
                              time + delay) + " deadline: " + str(deadline)))
                task.print_task()
...
class Hybrid(Strategy):
    ...
    def process_event(self, time, receiver, content, labels, log, node, flow_id, deadline, rtt_delay, status,
        ...
        if source == node and status == REQUEST:
            ret, reason = compSpot.admit_task(service['content'], service['labels'], time, flow_id, deadline, receiver,
                                              rtt_delay, self.controller, self.debug)
            if ret == False:
                if node == 'src_0':
        ...
        if receiver == node and status == REQUEST:
            ...
            rtt_delay += delay * 2
            self.controller.add_event(time + delay, receiver, service, labels, next_node, flow_id, deadline, rtt_delay,
                                      REQUEST)
            return
        ...
        if status == RESPONSE:
            ...
            if node == receiver:
                ...
                self.controller.add_event(time + delay, receiver, service, labels, next_node, flow_id, deadline,
                                          rtt_delay, RESPONSE)
            ...
            if node != source:
                ...
                if newTask is not None:
                    self.controller.add_event(newTask.completionTime, newTask.receiver, newTask.service, newTask.labels,
                                              node, newTask.flow_id, newTask.expiry, newTask.rtt_delay, TASK_COMPLETE,
                                              newTask)
            ...
            if (node != source and time + path_delay > deadline):
                print(("Error in HYBRID strategy: Request missed its deadline\nResponse at receiver at time: " + str(
                    time + path_delay) + " deadline: " + str(deadline)))
                task.print_task()
            ...
            delay = self.view.path_delay(node, next_node)
            deadline_metric = (
                        deadline - time - rtt_delay - compSpot.services[service['content']].service_time)  # /deadline
            if self.debug:
                print(("Deadline metric: " + repr(deadline_metric)))
            if self.view.has_service(node, service) and service["service_type"] is "proc" and \
                    (node in self.view.content_source(service, labels) or self.controller.has_message(node, labels,
                                                                                                      service[
                                                                                                          'content'])):
                if self.debug:
                    print("Calling admit_task")
                ret, reason = compSpot.admit_task(service['content'], labels, time, flow_id, deadline, receiver,
                                                  rtt_delay, self.controller, self.debug)
                if self.debug:
...
class MostFrequentlyUsed(Strategy):
    ...
    def process_event(self, time, receiver, content, labels, log, node, flow_id, deadline, rtt_delay, status,
        ...
        #  Request reached the cloud
        if source == node and status == REQUEST:
            ret, reason = compSpot.admit_task(service['content'], labels, time, flow_id, deadline, receiver, rtt_delay,
        ...
        # MFU
            ...
            next_node = path[1]
            delay = self.view.path_delay(node, next_node)
            deadline_metric = (deadline - time - rtt_delay - compSpot.services[
...
class StrictestDeadlineFirst(Strategy):
    ...
    def process_event(self, time, receiver, content, labels, log, node, flow_id, deadline, rtt_delay, status,
        ...
        if receiver == node and status == REQUEST:
            self.controller.start_session(time, receiver, service, labels, log, flow_id, deadline)
            source = self.view.content_source(service['content'], service['labels'])[
                len(self.view.content_source(service['content'], service['labels'])) - 1]
            path = self.view.shortest_path(node, source)
            next_node = path[1]
            delay = self.view.path_delay(node, next_node)
            rtt_delay += delay * 2
