...
class OptimalPlacementAndScheduling(Strategy):
        ...
        for ap in self.receivers:
            ...
            for service in range(0, self.view.num_services()):
                for node in list(self.compSpots.keys()):
                    ...
                    if cs.is_cloud:
                        continue
                    self.x_bar[service].value[ap, node] = self.x_bar[service].value[ap, node] * self.A[service][node][
                        ap]
        ...
        for service in range(0, self.view.num_services()):
            for ap in self.receivers:
                ...
                for node in list(self.compSpots.keys()):
                    ...
                    self.perAccessPerNodePerServiceProbability[ap][node][service] = self.x_bar[service].value[
                                                                                        ap, node] - math.floor(
                        self.x_bar[service].value[ap, node])
    ...
    def process_event(self, time, receiver, content, log, node, flow_id, deadline, rtt_delay, status):
        ...
        if source == node and status == REQUEST:
            ret, reason = compSpot.admit_task(service, time, flow_id, deadline, receiver, rtt_delay, self.controller,
                                              self.debug)
            if ret == False:
        ...
        # Request at an edge cloud
            ...
            aTask = Task(time, deadline, rtt_delay, node, service, compSpot.services[service].service_time, flow_id,
                         receiver)
            ret = compSpot.admit_task_with_probability(time, aTask, self.perAccessPerNodePerServiceProbability,
                                                       self.debug)
            ...
            if newTask is not None:
                ...
                self.controller.execute_service(newTask.flow_id, newTask.service, compSpot.node, time,
                                                compSpot.is_cloud)
                if self.debug:
                    print((str(time) + ". flow_id: " + str(flow_id) + " is being executed and to complete at: " + str(
                        newTask.completionTime)))
                    newTask.print_task()
            ...
            if self.debug:
                print((str(time) + ". response for flow_id: " + str(flow_id) + " reached the receiver at time: " + str(
                    time) + " deadline: " + str(deadline)))
            return
        else:
