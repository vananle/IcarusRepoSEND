...
class OptimalScheduling(Strategy):
    ...
    def __init__(self, view, controller, replacement_interval=10, debug=False, p=0.5, **kwargs):
        ...
        for service in range(0, view.num_services()):
            ...
            for ap in self.receivers:
                ap = int(ap[4:])
                self.perServiceReceiverRequestCounts[service][ap] = self.replacement_interval * (
                            rate / (len(self.receivers) * view.num_services()))
    ...
    def max_request_forwarded(self):
        objective = cp.Maximize(cp.sum_entries(
            cp.sum_entries(np.sum([cp.mul_elemwise(self.A[i].T, self.x_bar[i]) for i in range(self.S)]), axis=0),
            axis=1))

        capacityConstrain = cp.sum_entries(np.sum([cp.mul_elemwise(self.A[i], self.x_bar[i].T) for i in range(self.S)]),
    ...
    def compute_optimal_schedule(self):
        ...
        for service in range(0, self.view.num_services()):
            for ap in self.receivers:
                ap = int(ap[4:])
                self.avg_D[service].value[ap] = (1.0 * self.perServiceReceiverRequestCounts[service][
                    ap]) / self.replacement_interval
                self.std_D[service].value[ap] = self.avg_D[service].value[ap]
        ...
        for ap in self.receivers:
            ...
            for node in list(self.compSpots.keys()):
                ...
                for service in range(0, self.view.num_services()):
                    if self.debug:
                        print(("Count for ap: " + str(ap) + " service: " + str(service) + " is: " + str(
                            self.perServiceReceiverRequestCounts[service][ap])))
                    if self.perServiceReceiverRequestCounts[service][ap] > 0:
                        ...
                        self.perAccessPerNodePerServiceProbability[ap][node][service] = (self.x[ap, service].value[
                                                                                             node].item(0, 0)) / rate
                        if self.perAccessPerNodePerServiceProbability[ap][node][service] > 1.01:
                            raise ValueError("Invalid probability: " + str(
                                self.perAccessPerNodePerServiceProbability[ap][node][service]))
                    ...
                    if self.perServiceReceiverRequestCounts[service][ap] > 0:
                        for node in list(self.compSpots.keys()):
                            ...
                            if cs.is_cloud:
                                continue
                            node = int(node)
                            print(("Probability for access: " + str(ap) + " at node: " + str(node) + " service: " + str(
                                service) + " is " + str(self.perAccessPerNodePerServiceProbability[ap][node][service])))
    ...
    def process_event(self, time, receiver, content, log, node, flow_id, deadline, rtt_delay, status):
        ...
        if receiver == node and status == REQUEST:
            ...
            if upstream_node != source:
                ...
                aTask = Task(time, deadline, rtt_delay, upstream_node, service, cs.services[service].service_time,
                             flow_id, receiver, time + delay)
                ...
                self.controller.add_event(time + delay, receiver, service, upstream_node, flow_id, deadline, rtt_delay,
                                          REQUEST)
                if self.debug:
                ...
                self.controller.add_event(time + rtt_delay + serviceTime, receiver, service, receiver, flow_id,
            ...
            if newTask is not None:
                self.controller.add_event(newTask.completionTime, newTask.receiver, newTask.service, compSpot.node,
                                          newTask.flow_id, newTask.expiry, newTask.rtt_delay, TASK_COMPLETE)
                self.controller.execute_service(newTask.flow_id, newTask.service, compSpot.node, time,
                                                compSpot.is_cloud)
